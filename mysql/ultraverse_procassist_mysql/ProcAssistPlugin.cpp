...
void ProcAssistPlugin::handleEvent(THD *thd, mysql_event_class_t eventClass,
    ...
    try {
        ...
        /*
        fprintf(stderr, "eventClass %d, %d\n",
                  eventClass, commandId);
                  */
        
        if (eventClass == MYSQL_AUDIT_GENERAL_CLASS) {
            handleGeneralEvent(thd, reinterpret_cast<const mysql_event_general *>(event));
        }
        if (eventClass == MYSQL_AUDIT_QUERY_CLASS) {
            handleQueryEvent(thd, reinterpret_cast<const mysql_event_query *>(event));
        }
    } catch (std::exception &e) {
        fprintf(stderr, "== S O R R Y ==\n");
        fprintf(stderr, "e.what(): %s\n", e.what());
    }
...
void ProcAssistPlugin::handleQueryEvent(THD *thd, const mysql_event_query *event) {
    auto commandId = thd_sql_command(thd);
    long connectionId = event->connection_id;
    
    if (event->event_subclass & (MYSQL_AUDIT_QUERY_START | MYSQL_AUDIT_QUERY_NESTED_START)) {
        return;
    }
    
    
    std::string statement(event->query.str, event->query.length);
    
    fprintf(stderr, "[MYSQL_AUDIT_QUERY][%d][%ld]>> %s\n",
            commandId,
            connectionId,
            event->query.str);
...
void ProcAssistPlugin::writeProcLog(std::shared_ptr<ProcCall> procCall) {
    ...
    fprintf(stderr, "writing proc log\n");
