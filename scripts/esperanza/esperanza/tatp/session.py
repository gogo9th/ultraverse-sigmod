...
from esperanza.tatp.constants import DEFAULT_NUM_SUBSCRIBERS, DEFAULT_QUERY_COUNT
from esperanza.tatp.data_generator import TATPDataGenerator
from esperanza.tatp.workload import TATPWorkloadExecutor
from esperanza.utils.logger import get_logger


class TATPStandaloneSession:
    def __init__(
        self,
        name: str,
        scale: str,
        ...
        self.logger = get_logger(f"TATPStandaloneSession:{name}-{scale}")
    ...
    def prepare(self) -> None:
        ...
        self.logger.info("preparing TATP standalone session...")
    ...
    def execute_workload(self) -> dict:
        """Run the TATP workload."""
        conn = self._get_connection()
        executor = TATPWorkloadExecutor(conn, self.num_subscribers, self.query_count)
        stats = executor.run()
        conn.close()
        return stats
    ...
    def run_db_state_change(
        ...
        report_name = TATPStandaloneSession.report_name_from_stdout(stdout_name)
        env = os.environ.copy()
    ...
    def _execute_ddl(self) -> None:
        ddl_path = Path(__file__).parent.parent.parent / "tatp_sql" / "ddl-mysql.sql"
    ...
    def _load_data(self) -> None:
        conn = self._get_connection()
        generator = TATPDataGenerator(conn, self.num_subscribers)
    ...
    def _copy_procdefs(self) -> None:
        src = Path(__file__).parent.parent.parent / "procdefs" / "tatp"
        dst = Path(self.session_path) / "procdef"
        dst.mkdir(parents=True, exist_ok=True)

        if not src.exists():
            self.logger.warning(f"procdefs not found: {src}")
            return
