...
from esperanza.tpcc.constants import DEFAULT_QUERY_COUNT, DEFAULT_SCALE_FACTOR
from esperanza.tpcc.data_generator import TPCCDataGenerator
from esperanza.tpcc.workload import TPCCWorkloadExecutor
from esperanza.utils.logger import get_logger


class TPCCStandaloneSession:
    def __init__(
        self,
        name: str,
        scale: str,
        ...
        self.logger = get_logger(f"TPCCStandaloneSession:{name}-{scale}")
    ...
    def prepare(self) -> None:
        ...
        self.logger.info("preparing TPCC standalone session...")
    ...
    def execute_workload(self) -> dict:
        ...
        executor = TPCCWorkloadExecutor(conn, self.scale_factor, self.query_count)
        stats = executor.run()
        conn.close()
        return stats
    ...
    def run_db_state_change(
        ...
        report_name = TPCCStandaloneSession.report_name_from_stdout(stdout_name)
        env = os.environ.copy()
    ...
    def _execute_ddl(self) -> None:
        ddl_path = Path(__file__).parent.parent.parent / "tpcc_sql" / "ddl-mysql.sql"
    ...
    def _load_data(self) -> None:
        conn = self._get_connection()
        generator = TPCCDataGenerator(conn, self.scale_factor)
    ...
    def _copy_procdefs(self) -> None:
        src = Path(__file__).parent.parent.parent / "procdefs" / "tpcc"
