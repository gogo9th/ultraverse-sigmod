...
from esperanza.epinions.session import EpinionsStandaloneSession
...
def decide_rollback_gids(session: EpinionsStandaloneSession, ratio: float) -> list[int]:
...
def perform_state_change(
    session: EpinionsStandaloneSession,
...
def perform_full_replay(session: EpinionsStandaloneSession) -> None:
    logger = session.logger
