...
if [ $DB_MODE == "O" ]; then
  EXPORT_DATA=$EXPORT_PATH/twitter_export_base_origin
  MY_EXPORT_DATA=$EXPORT_PATH/twitter_export_my_origin
  DUMP_DATA=$EXPORT_PATH/twitter_dump_base_origin
  MY_DUMP_DATA=$EXPORT_PATH/twitter_dump_my_origin
else
  EXPORT_DATA=$EXPORT_PATH/twitter_export_base_modified
  MY_EXPORT_DATA=$EXPORT_PATH/twitter_export_my_modified
...
INSERT INTO twitter.added_tweets (uid,text,createdate) VALUES (123, 'KIMJH', '2019-12-08')"
...
USER_TABLE_NAME="twitter.added_tweets"
...
mysqldump -u$MYSQL_ID -p$MYSQL_PW -B twitter > $DUMP_DATA
...
mysqldump -u$MYSQL_ID -p$MYSQL_PW -B twitter > $MY_DUMP_DATA
print_log "export data...done"
