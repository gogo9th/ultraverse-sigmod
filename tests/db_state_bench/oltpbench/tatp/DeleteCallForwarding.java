...
//        "DELETE FROM " + TATPConstants.TABLENAME_CALL_FORWARDING + 
//        " WHERE s_id = (SELECT s_id FROM " + TATPConstants.TABLENAME_SUBSCRIBER + " WHERE sub_nbr = ?" + " LIMIT 1)" + " AND sf_type = ? AND start_time = ?"
        "DELETE FROM " + TATPConstants.TABLENAME_CALL_FORWARDING + 
        " WHERE (SELECT s_id FROM " + TATPConstants.TABLENAME_SUBSCRIBER + " WHERE s_id = ? and sub_nbr = ?" + " LIMIT 1)" + " >= 0 AND sf_type = ? AND start_time = ? AND s_id = ?"
    );
    ...
    public long run(Connection conn, String sub_nbr, byte sf_type, byte start_time) throws SQLException {
        ...
        //stmt.setLong(1, s_id);
			int s_id=Integer.parseInt(sub_nbr);  
        stmt.setInt(1, s_id);
        stmt.setString(2, sub_nbr);
        stmt.setByte(3, sf_type);
        stmt.setByte(4, start_time);
//System.out.println("MAP: " + sub_nbr + " -> " + s_id);
			stmt.setInt(5, s_id);
