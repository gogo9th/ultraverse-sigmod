...
public class InsertCallForwarding extends Procedure {
    ...
    public final SQLStmt insertCallForwarding = new SQLStmt(
        ...
        "INSERT INTO " + TATPConstants.TABLENAME_CALL_FORWARDING + " VALUES (?, !(SELECT s_id FROM " + TATPConstants.TABLENAME_SUBSCRIBER + " WHERE s_id = ? and sub_nbr = ?) + ?, ?, ?, ?)"
    );
    ...
    public long run(Connection conn, String sub_nbr, byte sf_type, byte start_time, byte end_time, String numberx) throws SQLException {
        ...
        */
        ...
        stmt.setLong(1, s_id);
        stmt.setLong(2, s_id);
        stmt.setString(3, sub_nbr);
        stmt.setByte(4, sf_type);
        stmt.setByte(5, start_time);
        stmt.setByte(6, end_time);
        stmt.setString(7, numberx);
        int rows_updated = -1;
