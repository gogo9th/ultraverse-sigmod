...
router.get('/:id', function (req, res, next) {
  if (req.session.user_id === undefined) {
    ...
    const payment_id = id_vec[0];
    const payment_name = id_vec[1];

    db_handler.runSelect(`SELECT * FROM Payments WHERE payment_id = ${payment_id}`).then(
      rows => {
        res.render('view-payment-detail', {
          title: 'View Payment Detail',
          base: false,
          payment_id: payment_id,
          payment_name: rows[0][0].payment_name,
          creator_id: rows[0][0].creator_id,
          recipient_id: rows[0][0].recipient_id,
          invoice_id: rows[0][0].invoice_id,
          bill_id: rows[0][0].bill_id,
          amount: rows[0][0].amount,
          payment_type: rows[0][0].payment_type,
...
router.get('/', function (req, res, next) {
  if (req.session.user_id === undefined) {
    ...
    db_handler.runSelect(`SELECT payment_id, payment_name FROM Payments`).then(
      rows => {
        var id_list = [];
        for (var i = 0, len = rows[0].length; i < len; i++) {
          var name = rows[0][i].payment_name;
          if (rows[0][0].payment_name === null) {
            name = 'null';
          }
          var id = rows[0][i].payment_id + ' - ' + name;
