...
async function post(req, res) {
  ...
  try {
    ...
    await db_handler.runTransaction(async function (conn, post) {
      ...
      await conn.query(`DELETE FROM Bills WHERE bill_id = ${post.bill_id}`);
    },
      post);

    // bill_id 가 없어졌기 때문에 리셋해야 함
    await db_handler.runSelect(`CALL CookieUpdate(@session_id, 1)`);
...
router.get('/', function (req, res, next) {
  if (req.session.user_id === undefined) {
    ...
    db_handler.runSelect(`SELECT bill_id FROM Bills WHERE creator_id = '${req.session.user_id}'`).then(
      rows => {
        var id_list = [];
