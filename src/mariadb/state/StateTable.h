...
namespace ultraverse::state {
    ...
    public:
        ...
        std::tuple<bool, QueryList>
        ReadUserDelLog(const std::string &log_bin_filepath, std::shared_ptr<StateQuery> user_del_transaction);
        ...
        bool CreateUndoTables(const std::vector<std::tuple<std::string, std::string>> &table_list,
                              std::vector<StateTableInfo> *info_list = NULL);
    ...
    private:
        ...
        static QueryList::iterator
        BinaryFindList(QueryList::iterator begin, QueryList::iterator end, const state_log_time &t);
        ...
        bool
        CreateUndoTablesLow(int command, MYSQL *mysql, const std::string &fullname, const state_log_time &query_time,
                            const state_log_time &undo_time, StateTableInfo *info);
        
        bool UndoVersionTable(MYSQL *mysql, const std::string &fullname, const state_log_time &query_time,
                              const state_log_time &undo_time);
        
        bool UndoBackupTable(MYSQL *mysql, const std::string &fullname, const state_log_time &query_time,
                             const state_log_time &undo_time);
        
        bool RunCreateTable(QueryList &list);
        
        bool RunCreateCommand(const std::vector<std::tuple<std::string, std::string>> &table_list, QueryList &list,
        ...
        GetAllTableName(const std::string &fullname, const state_log_time &query_time, const state_log_time &undo_time);
        ...
        void
        FindSubTable(const std::string &table, std::vector<std::string> &read_set, std::vector<std::string> &write_set);
        ...
        public:
            ...
            void add_referenced_set(const StateForeign &foreign) {
                auto iter = std::find_if(referenced_set.begin(), referenced_set.end(),
                                         [&foreign](const StateForeign &f) {
                                             if (f.table == foreign.table) {
                                                 auto c = StateUtil::return_sub_range_idx(f.columns, foreign.columns);
                                                 if (c.size() == f.columns.size())
                                                     return true;
