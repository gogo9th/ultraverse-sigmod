...
public:
  ...
  static bool SplitDBNameAndTableName(const std::string &in_fullname, std::string &out_dbname, std::string &out_tablename);
  ...
  static bool ReplaceQuery(const std::vector<std::tuple<std::string, std::string>> &oldname_list, std::string &query);

  static size_t RunQuery(StateQuery *query);
  static size_t RunBulkQuery(StateQuery *query);
  ...
  static std::string GetBackupTableName(MYSQL *mysql, const std::string &db, const std::string &table, const state_log_time &undo_time);

private:
  static std::shared_ptr<StateQuery> ParseThread(MYSQL *mysql, const std::string &db, const std::string &query, bool is_pass_all=false);
  static std::vector<std::string> GetBackupTableList(MYSQL *mysql, const std::string &db, const std::string &table);
  static std::vector<StateUserQuery::Range> GetVersionRange(MYSQL *mysql, const std::vector<std::string> &tlist);

  static size_t RunQueryLow(StateQuery *query, std::function<void(MYSQL *, StateQuery *)> init_func, std::function<void(MYSQL *, StateQuery *)> exit_func);
