...
inline static void update_token_pos(std::vector<size_t> &token_pos, size_t idx, size_t origin_string_size, size_t new_string_size)
{
...
inline void replace_single_table_name(const std::vector<std::tuple<std::string, std::string>> &oldname_list, std::string &query,
...
bool StateUserQuery::ReplaceQuery(const std::vector<std::tuple<std::string, std::string>> &oldname_list, std::string &query)
{
...
bool StateUserQuery::SplitDBNameAndTableName(const std::string &in_fullname, std::string &out_dbname, std::string &out_tablename)
{
...
size_t StateUserQuery::RunQueryLow(StateQuery *query, std::function<void(MYSQL *, StateQuery *)> init_func, std::function<void(MYSQL *, StateQuery *)> exit_func)
{
...
size_t StateUserQuery::RunBulkQuery(StateQuery *query)
{
...
std::string StateUserQuery::GetQueryTimeQuery(const state_log_time &time)
{
  ...
  snprintf(query, sizeof(query) - 1, STATE_QUERY_COMMENT_STRING" SET SESSION TIMESTAMP=%ld.%06lu", time.sec, time.sec_part);
  return query;
}
  ...
  switch (level)
  {
    case ISO_READ_UNCOMMITTED:
    {
      if (mysql_real_query(mysql,
          ...
          sizeof(STATE_QUERY_COMMENT_STRING" SET SESSION TRANSACTION ISOLATION LEVEL READ UNCOMMITTED")) != 0)
      {
    ...
    case ISO_READ_COMMITTED:
    {
      if (mysql_real_query(mysql,
          ...
          sizeof(STATE_QUERY_COMMENT_STRING" SET SESSION TRANSACTION ISOLATION LEVEL READ COMMITTED")) != 0)
      {
        error("[StateUserQuery::SetIsolationLevel] transaction query error [%s]", mysql_error(mysql));
        return false;
      }
    ...
    case ISO_REPEATABLE_READ:
    {
      if (mysql_real_query(mysql,
          ...
          sizeof(STATE_QUERY_COMMENT_STRING" SET SESSION TRANSACTION ISOLATION LEVEL REPEATABLE READ")) != 0)
      {
        error("[StateUserQuery::SetIsolationLevel] transaction query error [%s]", mysql_error(mysql));
        return false;
      }
    ...
    case ISO_SERIALIZABLE:
    {
      if (mysql_real_query(mysql,
          ...
          sizeof(STATE_QUERY_COMMENT_STRING" SET SESSION TRANSACTION ISOLATION LEVEL SERIALIZABLE")) != 0)
      {
        error("[StateUserQuery::SetIsolationLevel] transaction query error [%s]", mysql_error(mysql));
        return false;
      }
...
bool StateUserQuery::ResetQueryTime(MYSQL *mysql)
{
  ...
  return SetQueryTime(mysql,
                      state_log_time({std::chrono::duration_cast<std::chrono::seconds>(now.time_since_epoch()).count(),
                                      (ulong)(std::chrono::duration_cast<std::chrono::microseconds>(now.time_since_epoch()).count() % 1000000)}));
}
...
std::shared_ptr<StateQuery> StateUserQuery::ParseThread(MYSQL *mysql, const std::string &db, const std::string &query, bool is_pass_all)
{
...
std::string StateUserQuery::GetBackupTableName(MYSQL *mysql, const std::string &db, const std::string &table, const state_log_time &undo_time)
{
...
std::vector<std::string> StateUserQuery::GetBackupTableList(MYSQL *mysql, const std::string &db, const std::string &table)
{
...
std::mutex mutex;
std::vector<StateUserQuery::Range> StateUserQuery::GetVersionRange(MYSQL *mysql, const std::vector<std::string> &tlist)
{
