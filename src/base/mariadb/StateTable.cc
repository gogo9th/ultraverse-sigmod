...
bool StateTable::SetGroupID(const int id)
{
  ...
  if (0 != mysql_state_group_list(StateThreadPool::Instance().GetMySql(), group_id, &data, &data_len))
  {
...
{
  auto iter = std::lower_bound(list.begin(), list.end(), t, [](const StateQuery::st_transaction &s, const state_log_time &t) {
    return s.time < t;
  });
...
std::tuple<bool, StateTable::QueryList> StateTable::ReadUserDelLog(const std::string &log_bin_filepath, std::shared_ptr<StateQuery> user_del_transaction)
{
  is_user_query2 = true;
...
bool StateTable::ReadGroupLog(const std::string &log_bin_filepath)
{
  ...
  get_start_time().sec = start_time.sec;
  get_start_time().sec_part = start_time.sec_part;
...
int read_column_item_field(const char *ptr, size_t &curr, StateItem &col, size_t arg_count);
int read_column_item_condition(const char *ptr, size_t &curr, StateItem &col);
int read_column_item_function(const char *ptr, size_t &curr, StateItem &col);
int read_column_item(const char *ptr, size_t &curr, StateItem &col, uint8_t &data_count);
...
int read_column_item_field(const char *ptr, size_t &curr, StateItem &col, size_t arg_count)
{
  ...
  std::string key_name = "";
  if (get_key_column_name() != NULL) {
    key_name = get_key_column_name();
    auto pos = key_name.find_last_of('.');
    key_name = key_name.substr(pos + 1);
  }

  if (is_using_candidate() == false && col_name != key_name)
  {
  ...
  for (size_t i = 1; i < arg_count; ++i)
  {
    ...
    case en_column_item_func:
    {
      ...
      int ret = read_column_item_function(ptr, curr, *arg);
      if (ret < 0)
      {
        return -1;
      }
    ...
    case en_column_item_nul:
    {
      ...
      //키에 맞게 데이터 형변환
      else if (false == is_using_candidate() && false == data->ConvertData(get_key_column_type()))
      {
        col.data_list.pop_back();
      }
      ...
      if (data->IsSubSelect())
      {
        ...
        uint8_t tmp = 0;
        int ret = read_column_item(ptr, curr, *sub_col, tmp);
        total_count += tmp;
        if (ret < 0)
        {
...
int read_column_item_function(const char *ptr, size_t &curr, StateItem &col)
{
  ...
  {
    int ret = read_column_item_field(ptr, curr, *arg, item->arg_count);
    if (ret < 0)
    {
      return -1;
    }
...
int read_column_item(const char *ptr, size_t &curr, StateItem &col, uint8_t &data_count)
{
  ...
  case en_column_item_cond:
  {
    int ret = read_column_item_condition(ptr, curr, col);
    if (ret < 0)
    {
      return -1;
    }
  ...
  case en_column_item_func:
  {
    int ret = read_column_item_function(ptr, curr, col);
    if (ret < 0)
    {
      return -1;
    }
  ...
  case en_column_item_field:
  {
    int ret = read_column_item_field(ptr, curr, col, item->arg_count + 1);
    if (ret < 0)
    {
      return -1;
    }
...
StateTable::Query StateTable::ReadLogBlock(const char *ptr)
{
  ...
  for (uint8_t i = 0; i < hdr->foreign_count; ++i)
  {
    ...
    data->foreign_set.emplace_back(
        ...
        ptr + curr + sizeof(state_log_hdr_foreign) + foreign_hdr->foreign_name_length + foreign_hdr->foreign_table_length,
        foreign_hdr->columns_length,

        ptr + curr + sizeof(state_log_hdr_foreign) + foreign_hdr->foreign_name_length + foreign_hdr->foreign_table_length + foreign_hdr->columns_length,
        foreign_hdr->foreign_columns_length);

    curr += (sizeof(state_log_hdr_foreign) + foreign_hdr->foreign_name_length + foreign_hdr->foreign_table_length + foreign_hdr->columns_length + foreign_hdr->foreign_columns_length);
  }

  // debug("[StateTable::ReadLogBlock] %s", data->query.c_str());

  // candidate 탐색 모드이거나 설정한 키가 있을때만 사용
  if (is_using_candidate() || get_key_column_name() != NULL)
  {
...
void StateTable::RemoveList(QueryList &list, int create_command, int drop_command)
{
  auto forward_for = [&list, &drop_command]() {
    ...
    {
      for (auto trans_iter = (*iter)->transactions.begin(); trans_iter != (*iter)->transactions.end(); ++trans_iter)
      {
        ...
        if (trans_iter->command == drop_command)
        {
          ...
          if ((*iter)->transactions.size() == 0)
            return std::make_tuple(table, std::make_reverse_iterator(++iter), (*iter)->transactions.rend());
          else
            return std::make_tuple(table, std::make_reverse_iterator(++iter), std::make_reverse_iterator(++trans_iter));
        }
      }
    }
  ...
  auto reverse_for = [&list, &create_command](std::string table_name, QueryList::reverse_iterator rbegin, std::vector<StateQuery::st_transaction>::reverse_iterator trans_rbegin) {
      ...
      for (auto trans_iter = (*iter)->transactions.begin(); trans_iter != (*iter)->transactions.end(); ++trans_iter)
      {
        if (trans_iter->command == SQLCOM_DROP_DB)
        {
          ...
          if ((*iter)->transactions.size() == 0)
            ...
            return std::make_tuple(std::make_reverse_iterator(++iter), std::make_reverse_iterator(++trans_iter));
        }
      }
    }
  ...
  auto db_reverse_for = [&list, &target_db_name](QueryList::reverse_iterator rbegin, std::vector<StateQuery::st_transaction>::reverse_iterator trans_rbegin) {
...
std::vector<std::string> StateTable::GetAllTableName(const std::string &fullname, const state_log_time &query_time, const state_log_time &undo_time)
{
...
bool StateTable::UpdateUserTableList(const std::vector<std::string> &read_set, const std::vector<std::string> &write_set)
{
  ...
  if (write_set.size() > 0)
  {
    ...
    if (write_user_table_list.size() != tmp.size())
    {
      ...
      user_table_list.insert(user_table_list.end(), write_user_table_list.begin(), write_user_table_list.end());
    }
  }
      ...
      futures.emplace_back(StateThreadPool::Instance().EnqueueJob([](Query q) {
        ...
        if (q->range.size() > 0)
        {
          ...
          for (auto &i : q->foreign_set)
          {
            ...
            if (i.table == get_key_column_table_name())
            {
...
void StateTable::UpdateUserTableListFromLog_1()
{
  ...
  if (user_transaction != NULL)
  {
    ...
    current_range_set.insert(current_range_set.end(), user_transaction->range.begin(), user_transaction->range.end());
  }
  ...
  if (group_transaction != NULL)
  {
    ...
    current_range_set.insert(current_range_set.end(), group_transaction->range.begin(), group_transaction->range.end());
  }
  ...
  */
  ...
  if (is_using_candidate() == false && get_key_column_name() == NULL)
  {
    // candidate 탐색 모드이거나 설정한 키가 있을때만 사용
    current_range_set.clear();
  }
...
void StateTable::UpdateUserTableListFromTableLog()
{
  auto process_add_user_table = [this](const std::vector<std::string> &read_set,
    ...
    for (auto &t : read_set)
    {
      if (write_user_table_list.end() != std::find(write_user_table_list.begin(), write_user_table_list.end(), t))
      {
...
void StateTable::FindSubTable(const std::string &table, std::vector<std::string> &read_set, std::vector<std::string> &write_set)
{
  ...
  case SQLCOM_UPDATE_MULTI:
  {
    ...
    for (auto &sub_ref : *iter->second.get_referenced_set())
    {
      ...
      if (sub_ref.update_type == en_foreign_key_opt_check)
      {
        ...
        if (std::find(curr_foreign.columns.begin(), curr_foreign.columns.end(), "*") != curr_foreign.columns.end())
          new_columns = sub_ref.columns;
        else
        {
          new_columns.clear();

          new_columns_idx = StateUtil::return_sub_range_idx(curr_foreign.columns, sub_ref.foreign_columns);
...
bool StateTable::ValidTableLogLow()
{
  ...
  debug("[StateTable::ValidTableLogLow] procedure : %lu, valid : %lu", procedure_list.size(), valid_procedure_list.size());
  debug("[StateTable::ValidTableLogLow] view : %lu, valid : %lu", view_list.size(), valid_view_list.size());
  debug("[StateTable::ValidTableLogLow] trigger : %lu, valid : %lu", trigger_list.size(), valid_trigger_list.size());
...
bool StateTable::AnalyzeLog()
{
  ...
  if (group_transaction != NULL)
  {
    ...
    auto &hash_query_time = get_hash_query_time();
...
bool StateTable::UndoVersionTable(MYSQL *mysql, const std::string &fullname, const state_log_time &query_time, const state_log_time &undo_time)
{
  ...
  if (current_range_set.size() > 0 && fullname == get_key_column_table_name())
  {
    std::stringstream query;
    ...
    for (auto &i : current_range_set)
    {
      ...
      */
      ...
      query << STATE_QUERY_COMMENT_STRING " INSERT INTO " << STATE_CHANGE_DATABASE
            ...
            << " FOR SYSTEM_TIME AS OF TIMESTAMP '" << StateUtil::format_time(undo_time.sec, "%Y-%m-%d %H:%M:%S") << '.' << std::setfill('0') << std::setw(6) << undo_time.sec_part << "'"
            << " WHERE (" << where << "));";
      if (mysql_query(mysql, query.str().c_str()) != 0)
      {
      ...
      query << STATE_QUERY_COMMENT_STRING " INSERT INTO " << STATE_CHANGE_DATABASE
            << ".`" << undo_dbname << "." << undo_tablename
            << "` ( SELECT * FROM " << StateUserQuery::MakeFullTableName(curr_fullname)
            << " FOR SYSTEM_TIME AS OF TIMESTAMP '" << StateUtil::format_time(get_end_time().sec, "%Y-%m-%d %H:%M:%S") << '.' << std::setfill('0') << std::setw(6) << get_end_time().sec_part << "'"
            << " WHERE !(" << where << "));";
      if (mysql_query(mysql, query.str().c_str()) != 0)
      {
        error("[StateTable::UndoVersionTable] query error [%s] [%s]", query.str().c_str(), mysql_error(mysql));
...
bool StateTable::UndoBackupTable(MYSQL *mysql, const std::string &fullname, const state_log_time &query_time, const state_log_time &undo_time)
{
  std::string curr_fullname = GetEndTableName(fullname, query_time);
  std::string undo_fullname = GetBeginTableName(fullname, query_time);
  ...
  std::string backup_fullname = StateUserQuery::GetBackupTableName(mysql, backup_dbname, backup_tablename, undo_time);
...
bool StateTable::CreateUndoTablesLow(int command, MYSQL *mysql, const std::string &fullname, const state_log_time &query_time, const state_log_time &undo_time, StateTableInfo *info)
{
...
bool StateTable::CreateUndoTables(const std::vector<std::tuple<std::string, std::string>> &table_list, std::vector<StateTableInfo> *info_list)
{
  //cleanup
  ...
  auto func = [&](int command, const std::string &fullname, const state_log_time &query_time, const state_log_time &undo_time, StateTableInfo *info) {
  ...
  if (info_list != NULL)
  {
    ...
    {
      futures.emplace_back(StateThreadPool::Instance().EnqueueJob(func, i.second.command, i.first, i.second.time, start_time, &info_list->at(idx++)));
    }
  }
  else
  {
    for (auto &i : table_map)
    {
      futures.emplace_back(StateThreadPool::Instance().EnqueueJob(func, i.second.command, i.first, i.second.time, start_time, nullptr));
    }
  }
...
bool StateTable::RunCreateCommand(const std::vector<std::tuple<std::string, std::string>> &table_list, QueryList &list, int command, bool is_check)
{
  ...
  for (auto &i : list)
  {
    for (auto &t : i->transactions)
    {
      ...
      if (is_check)
      {
        if (t.write_set.size() == 0 || std::find(user_table_list.begin(), user_table_list.end(), t.write_set[0]) == user_table_list.end())
        {
          continue;
        }
        debug("[StateTable::RunCreateCommand] %s %s", StateUtil::GetCommandName(t.command), t.write_set[0].c_str());
      }
      else
      {
        StateUserQuery::ReplaceQuery(table_list, t.query);
        debug("[StateTable::RunCreateCommand] %s %s", StateUtil::GetCommandName(t.command), t.query.substr(0, t.query.find('\n')).c_str());
      }
      ...
      futures.emplace_back(StateThreadPool::Instance().EnqueueJob(
          [](StateQuery::st_transaction &t) -> size_t {
            ...
            if (false == StateUserQuery::SetQueryTime(mysql, t.time))
            {
              ...
              StateThreadPool::Instance().PushMySql(mysql, false);
              return 1;
            }
            ...
            {
              error("[StateTable::RunCreateCommand] query error [%s] [%s]", new_query.c_str(), mysql_error(mysql));
              StateThreadPool::Instance().PushMySql(mysql, false);
              return 1;
            }
