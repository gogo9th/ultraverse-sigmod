...
$ # Make sure MySQL source is available at ./mysql-server
$ #   or pass -DULTRAVERSE_MYSQLD_SRC_PATH=/path/to/mysql-server when configuring Ultraverse.
$ # The MySQL source tree must be configured at least once to generate
$ #   include/my_config.h and include/mysql_version.h (autogenerated headers).
$ # Example (run once inside the MySQL source tree):
