...
void StateGraphBoost::CreateEdge(size_t node_idx, size_t pnode_idx, const std::string &table)
{
  ...
  for (auto iter = in.first; iter != in.second; ++iter)
  {
    ...
    if (std::find(source_query->read_set.begin(), source_query->read_set.end(), table) != source_query->read_set.end())
    {
      return;
    }
    if (std::find(source_query->write_set.begin(), source_query->write_set.end(), table) != source_query->write_set.end())
    {
      return;
    }
  }
...
void StateGraphBoost::MakeOutputFilename(const std::string &type, const std::string &filepath)
{
  ...
  else
  {
    ...
    public:
      ...
      static void print_map(std::ostream &outStream, const std::map<std::string, std::vector<std::string>> &m)
      {
        if (m.size() == 0)
        {
          return;
...
void StateGraphBoost::DotConvert(const std::string &type, const std::string &prev_filepath, const std::string &filepath)
{
